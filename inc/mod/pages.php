...
function mod_move(Context $ctx, $channel, $originBoard, $postID) {
    ...
    if (isset($_POST['board'])) {
        ...
        // Update cite rows in both directions. Some stale entries may still reference the source board/post
        // because they were recorded before the thread move or before the source thread is deleted.
        try {
            foreach ($newIDs as $old_id => $new_id) {
                // Any cite row where this post is the target should now resolve to the moved post on the target board.
                ...
                if (!empty($citing_posts)) {
                    ...
                    error_log("mod_move: Updated " . count($citing_posts) . " incoming cites for post $old_id -> $new_id on board $originBoardURI -> $targetBoard");
                ...
                // Any cite row created from the source board for the old moved post must also be remapped to the target board.
                $source_update = prepare('UPDATE ``cites`` SET `board` = :target_board, `post` = :new_id WHERE `board` = :origin_board AND `post` = :old_id');
                $source_update->bindValue(':target_board', $targetBoard);
                $source_update->bindValue(':new_id', $new_id, PDO::PARAM_INT);
                $source_update->bindValue(':origin_board', $originBoardURI);
                $source_update->bindValue(':old_id', $old_id, PDO::PARAM_INT);
                $source_update->execute() or error(db_error($source_update));
            }
        } catch (Exception $e) {
